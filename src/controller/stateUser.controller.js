...
const {existeEstado,buscarId,modificarRoles} = require('../service/stateUser.service')
...
async function crearNuevoEstado(req= request, res = response){
    ...
    const estado_encontrado = await existeEstado(name); 
     if(estado_encontrado) return res.status(409). json({msg: "Ya existe el rol ingresado!"}); 
     
     const nuevoEstado = new estadousuario({name, state});
